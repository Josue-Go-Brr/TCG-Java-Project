...
/**
 * Instantiates deck builder card tiles and sizes the scrollable grid like the library screen.
 */
public class DeckBuilderGridRenderer {
	...
	public void render(List<BaseCarte> cards) {
		...
		if (cardGrid == null || cardTileScene == null) {
			GD.INSTANCE.pushWarning("[DeckBuilder] Missing grid or tile scene.");
			return;
		}
		...
		for (BaseCarte card : cards) {
			...
			if (!(instance instanceof DeckBuilderCardTileController tileController)) {
				...
				GD.INSTANCE.pushWarning("[DeckBuilder] card_tile instance is not DeckBuilderCardTileController.");
				continue;
			}
	...
	private void updateScrollableContentHeight(int cardCount) {
		...
		GD.INSTANCE.print(
				"[DeckBuilder][GridHeight] cards=" + cardCount
						+ ", columns=" + columns
						+ ", rows=" + rows
		...
		if (parent instanceof Control parentControl) {
			...
			GD.INSTANCE.print("[DeckBuilder][GridHeight] parent min size set to " + finalHeight);
		}
	}
}
